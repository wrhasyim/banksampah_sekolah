    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('jenis-sampah.update', $item->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div class="md:col-span-1">
                            ...
                            <x-text-input id="nama_sampah" class="block mt-1 w-full" type="text" name="nama_sampah" :value="old('nama_sampah', $item->nama_sampah)" required autofocus />
                        ...
                        <div class="md:col-span-1">
                            ...
                            <select name="satuan" id="satuan" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm" required>
                                <option value="pcs" {{ old('satuan', $item->satuan) == 'pcs' ? 'selected' : '' }}>Pcs (Buah)</option>
                                <option value="kg" {{ old('satuan', $item->satuan) == 'kg' ? 'selected' : '' }}>Kg (Kilogram)</option>
                            </select>
                            <x-input-error :messages="$errors->get('satuan')" class="mt-2" />
                        </div>
                        <div class="md:col-span-1">
                            <x-input-label for="harga_per_satuan" value="Harga Beli (dari Siswa)" />
                            <x-text-input id="harga_per_satuan" class="block mt-1 w-full" type="number" name="harga_per_satuan" :value="old('harga_per_satuan', $item->harga_per_satuan)" required />
                        ...
                        <div class="md:col-span-1">
                            ...
                            <x-text-input id="harga_jual" class="block mt-1 w-full" type="number" name="harga_jual" :value="old('harga_jual', $item->harga_jual)" required />
